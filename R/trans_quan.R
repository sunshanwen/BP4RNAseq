...
extract <- function()
{
  files <- list.files(pattern = ".*\\.gtf$", recursive = T, full.names = T)
  outputs <- c()
  for (file in files)
  {
    ...
    cmd = paste("awk -F \'\\t\' \'$3 == \"transcript\" {print $9}\'", file, "| awk -F \';\' \'{print $2, $5, $6}\' | awk \'BEGIN{OFS = \",\"; print \"transcript_id\", \"FPKM\", \"TPM\"} {print $2, $4, $6}\' >", output)
    # cat(cmd, "\n")
    system(cmd)
  }
  return(outputs)
...
trans_quan <- function()
{
  ...
  unlink(files)
  files <- list.files(pattern = ".*bam$", recursive = T, full.names = T)
  unlink(files)
  files <- list.files(pattern = ".*ht2$", recursive = T, full.names = T)
  unlink(files)
  files <- list.files(pattern = ".*table$", recursive = T, full.names = T)
  unlink(files)
