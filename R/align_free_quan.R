...
tx2gene <- function() {
    ...
    # cmd1 <- paste('egrep -v '^#|^$'', annotation, '| awk -F '\t' '$3 ~ /RNA/ {print $9}' | awk -F ';' 'BEGIN{OFS = \'=\';} {print $1, $2;}' | awk -F
    # '=' 'BEGIN{OFS = \'-\';}{print $NF, $2;}'| grep 'gene' | awk -F '-' 'BEGIN{OFS = \',\';print \'gene_id\', \'transcript_id\'}{print $2, $4}'
    # > tx2gene.csv')
    if (length(annotation)) {
        ...
        if (file.info("raw_tx2gene.csv")$size == 0) {
            cmd1 <- paste("egrep -v '^#|^$'", annotation, "| cut -f 9 | grep ID=transcript | awk -F ';' 'BEGIN{OFS = \":\";} {print $1, $2;}' | awk -F ':' 'BEGIN{OFS = \",\"} {print $NF, $2}' > raw_tx2gene.csv")
            # cat(cmd1)
            system(cmd1)
        }
...
align_free_quan <- function(pair, genome, transcript, annotation, threads = 4, salmon_index_add = NULL, salmon_quan_add = NULL) {
    status <- tryCatch(system2(command = "which", args = "salmon", stderr = FALSE, stdout = FALSE), error = function(err) {
        1
            ...
            if (pair == "paired") {
                ...
                if (length(read)) {
                  for (f in read) {
                    ...
                    cmd4 <- paste("salmon quant -p", threads, "-i salmon_index -l A", "-1", read1, "-2", read2, "--validateMappings -o", out, salmon_quan_add)
                    # cat(cmd4, '\n')
                ...
                if (length(read)) {
                  ...
                  for (f in read) {
                    ...
                    cmd4 <- paste("salmon quant -p", threads, "-i salmon_index -l A -r", f, "--validateMappings -o", out, salmon_quan_add)
