    ...
    if(answer3 == "y")
    {
      for(i in samples)
      {
        if(pair[pair$samples == i, 2] == "pair")
        {
          ...
          cmd = paste("cutadapt -a", sequence1, "-A", sequence2,"-o", outfile1, "-p", outfile2,"-j ", threads, infile1, infile2)
          ...
          if(adap_3or5 == "3"){
            cmd = paste("cutadapt -a", sequence, "-o", outfile, "-j ", threads, infile)
            system(cmd)
            unlink(c(infile))
          }else if(adap_3or5 == "5"){
            cmd = paste("cutadapt -g", sequence, "-o", outfile, "-j ", threads, infile)
            system(cmd)
            unlink(c(infile))
          }else if(adap_3or5 == "b"){
            cmd = paste("cutadapt -b", sequence, "-o", outfile, "-j ", threads, infile)
            system(cmd)
            unlink(c(infile))
      ...
      for(i in samples)
      {
        if(pair[pair$samples == i, 2] == "pair")
        {
          ...
          if(adapter == "Illumina Universal Adapter"){
            cmd = paste("cutadapt -a AGATCGGAAGAG -A AGATCGGAAGAG", "-o", outfile1, "-p", outfile2,"-j ", threads, infile1, infile2)
            #cat(cmd,"\n")#print the current command
            system(cmd)
            unlink(c(infile1, infile2))
          }else if(adapter == "Illumina Small RNA 3' Adapter"){
            cmd = paste("cutadapt -a TGGAATTCTCGG -A TGGAATTCTCGG", "-o", outfile1, "-p", outfile2,"-j ", threads, infile1, infile2)
            #cat(cmd,"\n")#print the current command
            system(cmd)
            unlink(c(infile1, infile2))
          }else if(adapter == "Nextera Transposase Sequence"){
            cmd = paste("cutadapt -a CTGTCTCTTATA -A CTGTCTCTTATA", "-o", outfile1, "-p", outfile2,"-j ", threads, infile1, infile2)
            #cat(cmd,"\n")#print the current command
            system(cmd)
            unlink(c(infile1, infile2))
          }else if(adapter == "SOLID Small RNA Adapter"){
            cmd = paste("cutadapt -a CGCCTTGGCCGT -A CGCCTTGGCCGT", "-o", outfile1, "-p", outfile2,"-j ", threads, infile1, infile2)
            #cat(cmd,"\n")#print the current command
            system(cmd)
            unlink(c(infile1, infile2))
          }else if(adapter == "Illumina Small RNA 5' Adapter"){
            cmd = paste("cutadapt -a GATCGTCGGACT -A GATCGTCGGACT", "-o", outfile1, "-p", outfile2,"-j ", threads, infile1, infile2)
            #cat(cmd,"\n")#print the current command
            system(cmd)
            unlink(c(infile1, infile2))
          ...
          outfile <-paste0("Adapter_trimmed_", i, ".fastq")### maybe need to add "_"
          if(adapter == "Illumina Universal Adapter"){
            cmd = paste("cutadapt -a AGATCGGAAGAG", "-o", outfile, "-j ", threads, infile)
            #cat(cmd,"\n")#print the current command
            system(cmd)
            unlink(c(infile))
          }else if(adapter == "Illumina Small RNA 3' Adapter"){
            cmd = paste("cutadapt -a TGGAATTCTCGG", "-o", outfile, "-j ", threads, infile)
            #cat(cmd,"\n")#print the current command
            system(cmd)
            unlink(c(infile))
          }else if(adapter == "Nextera Transposase Sequence"){
            cmd = paste("cutadapt -a CTGTCTCTTATA", "-o", outfile, "-j ", threads, infile)
            #cat(cmd,"\n")#print the current command
            system(cmd)
            unlink(c(infile))
          }else if(adapter == "SOLID Small RNA Adapter"){
            cmd = paste("cutadapt -a CGCCTTGGCCGT", "-o", outfile,"-j ", threads, infile)
            #cat(cmd,"\n")#print the current command
            system(cmd)
            unlink(c(infile))
          }else if(adapter == "Illumina Small RNA 5' Adapter"){
            cmd = paste("cutadapt -g GATCGTCGGACT", "-o", outfile,"-j ", threads, infile)
            #cat(cmd,"\n")#print the current command
            system(cmd)
            unlink(c(infile))
          }else stop("Wrong adapter. Exit.")
