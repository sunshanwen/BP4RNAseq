...
sra_download <- function(accession, dir = getwd()) {
    if (Sys.info()["sysname"] == "Windows") {
        ...
        if (status == 0) {
            system2(command = "bash", args = "-c vdb-config --interactive & read -t 3; kill $!")
            
            ## Get the default download directory
            dir.download <- NULL
            
            root.dir <- system2(command = "vdb-config", args = "| grep '<root>' | cut -d '>' -f 2 | cut -d '<' -f 1")
            root.exist <- NULL
            ...
            if (length(dir.download) == 0) {
                dir.download <- system2(command = "vdb-config", args = "| grep '<HOME>' | cut -d '>' -f 2 | cut -d '<' -f 1")
                root.exist <- FALSE
            }
